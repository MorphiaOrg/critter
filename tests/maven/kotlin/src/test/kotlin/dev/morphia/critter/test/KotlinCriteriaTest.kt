...
class KotlinCriteriaTest : BottleRocketTest() {
    ...
    fun andQueries(state: String, datastore: Datastore) {
        ...
        val query1 = datastore.find(Person::class.java)
            .filter(
                and(
                    PersonCriteria.first().eq("Mike"),
                    PersonCriteria.last().eq("Tyson")
                )
            )
        val query2 = datastore.find(Person::class.java)
            .filter(
                Filters.eq(PersonCriteria.first, "Mike"),
                Filters.eq(PersonCriteria.last, "Tyson")
            )
    ...
    fun embeds(state: String, datastore: Datastore) {
        ...
        Assert.assertEquals(
            list[0].addresses[0].city,
            "NYC",
            list.stream().map { obj: Invoice -> obj.id }.collect(
                Collectors.toList()
            ).toString()
        )
        Assert.assertEquals(
            list[0],
            invoice,
            list.stream().map { obj: Invoice -> obj.id }
                .toString()
        )
    ...
    fun invoice(state: String, ds: Datastore) {
        ...
        ds.save(
            Invoice(
                LocalDateTime.of(2012, 12, 21, 13, 15), john,
        ...
        val query = ds.find(Invoice::class.java)
            .filter(InvoiceCriteria.person().eq(john))
        var invoice = query.first() as Invoice
        val doe = ds.find(Invoice::class.java)
            .filter(Filters.eq(InvoiceCriteria.person, john))
            .first() as Invoice
        ...
        invoice = ds.find(Invoice::class.java)
            .filter(Filters.eq(addresses().city().path, "Chicago"))
            .first() as Invoice
        Assert.assertNotNull(invoice)
        val critter = ds.find(Invoice::class.java)
        ...
        ds.save(created)
        Assert.assertTrue(created.isPrePersist)
        Assert.assertTrue(created.isPostPersist)
        ...
        Assert.assertTrue(loaded.isPreLoad)
        Assert.assertTrue(loaded.isPostLoad)
    }
    ...
    fun orQueries(state: String, datastore: Datastore) {
        ...
        val criteria = datastore.find(Person::class.java)
            .filter(
                Filters.or(
                    PersonCriteria.last().eq("Bloomberg"),
                    PersonCriteria.last().eq("Tyson")
                )
            )
    ...
    fun removes(state: String, datastore: Datastore) {
        ...
        var criteria = datastore.find(Person::class.java)
            .filter(
                PersonCriteria.last().regex()
                    .pattern("Last2")
            )
    ...
    fun updates(state: String, datastore: Datastore) {
        ...
        query.delete()
        query.filter(
            PersonCriteria.first().eq("Jim"),
            PersonCriteria.last().eq("Beam")
        )
        Assert.assertEquals(
            query.update(
                age().set(30L)
        ...
        Assert.assertEquals(
            datastore.find(Person::class.java)
                .first()?.age, 31L
        )
